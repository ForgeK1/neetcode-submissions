class Solution 
{
    public boolean isAnagram(String s, String t) 
    {
        ...
        //Creates a HashMap to store each character for each string
        HashMap<String, Integer> sH = new HashMap<String, Integer>();
        HashMap<String, Integer> tH = new HashMap<String, Integer>();

        //A for loop to add each character to their respective HashMaps
        for(int i = 0; i < s.length(); i++)
        {
            //Debug code
                //System.out.println("For loop runs");

            //Checks if the character already exists in the HashMap in order to increment the value
            if(sH.containsKey(s.charAt(i) + ""))
            {
                sH.replace(s.charAt(i) + "", sH.get(s.charAt(i) + "") + 1);
            }
            //Otherwise, adds the character to the HashMap
            else
            {
                sH.put(s.charAt(i) + "", 1);
            }
        ...
        //A for each loop to check the value from each character in both HashMaps
        for(Map.Entry<String, Integer> entry : sH.entrySet())
        {
            //If values don't match, then the two strings are not equal to each other
